...
open(IN,"zsi_hpg_books.xml") or die "can't open zsi_hpg_books.xml\n";

my $dbh=DBI->connect("DBI:mysql:database=$db;host=$host","$usr","$pwd");

$sth1=$dbh->prepare("CREATE TABLE hpg_books_list(
book_id varchar(4), 
level int(2),
title varchar(1000),
authid varchar(200),
...
sub insert_to_db()
{
	...
	$sth2=$dbh->prepare("insert into hpg_books_list values('$book_id','$level','$title','$authid','$authors','$page','$page_end','$edition','$volume','$part','$type','$year','$month','')");
	$sth2->execute();
	$sth2->finish();
}
