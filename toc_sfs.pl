...
open(IN,"zsi_sfs_toc.xml") or die "can't open zsi_sfs_toc.xml\n";

my $dbh=DBI->connect("DBI:mysql:database=$db;host=$host","$usr","$pwd");

$sth1=$dbh->prepare("CREATE TABLE sfs_book_toc(book_id varchar(4), 
level int(2),
title varchar(1000),
authid varchar(200),
...
sub insert_to_db()
{
	...
	$sth2=$dbh->prepare("insert into sfs_book_toc values('$book_id','$level','$title','$authid','$authors','$page','$type','')");
	$sth2->execute();
	$sth2->finish();
}
